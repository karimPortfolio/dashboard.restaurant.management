                ...
                >
                    ...
                    <div class="flex flex-col justify-center items-center">
                        <q-avatar class="w-20 h-20">
                            <q-img
                                :src="newCategory.image ?? '/img/avatar.jpg'"
                            />
                        </q-avatar>
                        <div>
                            <q-btn
                    ...
                    <div>
                        <custom-select
                            v-model="newCategory.parent_category"
                            resource="categories"
                            label="Main Category"
...
const { create, creating, validation } = useResourceCreate("categories", {
    config: {
        headers: {
...
const handleCreate = async () => {
    newCategory.value.image = files.value ? files.value[0] : null;
    await create(newCategory.value);
    emit("created");
};
...
watch(
    ...
    (value) => {
        if (files.value && files.value.length) {
            newCategory.value.image = URL.createObjectURL(files.value[0]);
        }
    }
);
</script>
